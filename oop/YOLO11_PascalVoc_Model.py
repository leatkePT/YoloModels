...
# Προσθήκη των γρήγορων μετρικών
...
# ---------------------------------------------------------
# Main API Class: YOLO11_PascalVOC
# ---------------------------------------------------------
class YOLO11_PascalVOC:

    def __init__(self):
        self.name = 'YOLO11_Nano_PascalVOC'
        ...
        self.pascal_years = ["VOC2007", "VOC2012"]
        self.base_dir = "../datasets/pascal_voc/VOCdevkit"

        # Pretrained Weights & Logs
        self.coco_pretrained_weights = "../weights/yolo11n_coco_pretrained.weights.h5"

        # ΕΝΗΜΕΡΩΜΕΝΟ PATH ΒΑΡΩΝ (Να ταιριάζει με το Oxford Pets)
        self.baseline_model_weights = "../trained_models/yolo11n/pascal/yolov11_nano_PASCAL_VOC_0712_11n_final.weights.h5"

        # ΕΝΗΜΕΡΩΜΕΝΟ PATH ΛΟΓΚΑΡΙΣΜΑΤΟΣ
        self.log_dir = "../logs/oop_experiments/pascal"
    ...
    def data_preprocessing(self):
        ...
        for year in self.pascal_years:
            img_dir = os.path.join(self.base_dir, year, "JPEGImages")
            ann_dir = os.path.join(self.base_dir, year, "Annotations")
            if os.path.exists(img_dir) and os.path.exists(ann_dir):
                for img_name in os.listdir(img_dir):
                    if img_name.lower().endswith('.jpg'):
                        xml_name = img_name.rsplit('.', 1)[0] + '.xml'
                        self.all_image_data.append((os.path.join(img_dir, img_name), os.path.join(ann_dir, xml_name)))

        if not self.all_image_data:
            print(f"[ERROR] No images found at {self.base_dir}. Please verify dataset path.")
            sys.exit(1)

        random.seed(42)
        random.shuffle(self.all_image_data)
        split_index = int(len(self.all_image_data) * 0.8)

        # Caching XMLs for speed
        xml_cache = {}
        for img_path, xml_path in self.all_image_data:
        ...
        def load_letterbox(data_tuple):
            img, boxes, classes = load_raw(data_tuple)
            if img is None: return None, None, None
            shape = img.shape[:2]
        ...
        def data_gen():
            for i, data_tuple in enumerate(self.all_image_data):
                is_val = 0 if i < split_index else 1
                if is_val == 0:
                    if random.random() > 0.3:
                        image, boxes, classes = load_mosaic(data_tuple)
                    else:
                        image, boxes, classes = load_letterbox(data_tuple)
        ...
        print(f"[INFO] Dataset successfully loaded. Total samples: {len(self.all_image_data)}")
    ...
    def fine_tune(self, path_to_save, new_model=None, print_summary=False, new_fine_tune_epochs=None):
        print("\n" + "=" * 50)
        print("[INFO] Initiating Pruning Recovery (Fine-Tuning) phase for PASCAL VOC...")
        ...
        def decode_preds(model_outputs):
            ...
            nms_out = tf.image.combined_non_max_suppression(
                ...
                iou_threshold=0.60, score_threshold=0.01, clip_boxes=False # ΔΙΟΡΘΩΣΗ: ΠΑΡΑΜΕΝΕΙ ΣΕ 0.01
            )
        ...
        for epoch in range(fine_tune_epochs):
            ...
            # PyCOCOTools Fast Evaluation
            val_loss_total, num_val_batches = 0.0, 0
            gt_annotations, dt_annotations, images_info = [], [], []
            ...
            for val_images, val_boxes, val_classes, t1, t2, t3 in pbar_val:
                ...
                y_true_boxes_np, y_true_classes_np = val_boxes.numpy(), val_classes.numpy()
                y_pred_boxes_np, y_pred_classes_np, y_pred_scores_np = y_pred_dict["boxes"].numpy(), y_pred_dict[
                    "classes"].numpy(), y_pred_dict["confidence"].numpy()

                for b in range(y_true_boxes_np.shape[0]):
                    img_id_counter += 1
                    images_info.append(
                        {"id": img_id_counter, "width": self.target_size[0], "height": self.target_size[1]})

                    for i in range(100):
                        cls = y_true_classes_np[b, i]
                        if cls == -1: continue
                        cx, cy = y_true_boxes_np[b, i, 0] * self.target_size[0], y_true_boxes_np[b, i, 1] * \
                                 self.target_size[1]
                        w, h = y_true_boxes_np[b, i, 2] * self.target_size[0], y_true_boxes_np[b, i, 3] * \
                               self.target_size[1]
                        gt_annotations.append({
                            "id": ann_id_counter, "image_id": img_id_counter, "category_id": int(cls),
                            "bbox": [float(cx - w / 2.0), float(cy - h / 2.0), float(w), float(h)],
            ...
            with redirect_stdout(f):
                categories = [{"id": i, "name": name} for name, i in self.class_to_id.items()]
                coco_gt = COCO()
                ...
                if len(dt_annotations) > 0:
                    ...
                    cocoEval.accumulate()
                    cocoEval.summarize()
                    map_50_95, map_50 = float(cocoEval.stats[0]), float(cocoEval.stats[1])
                else:
    ...
    def get_best_epoch_info(self, history, metrics=None):
        ...
        new_history['best_epoch'] = int(best_epoch_ix + 1)
        return new_history

    def extract_monitoring_metric_value(self, model=None):
        if not self.model_history:
            print("[WARNING] No training history found. The method relies on the history dictionary.")
            return 0.0
        ...
        if monitor_metric in best_info:
            ...
            print(f"[EVALUATION] Retrieved retained metric '{monitor_metric}' from history: {metric_val:.4f}")
            return metric_val
        else:
