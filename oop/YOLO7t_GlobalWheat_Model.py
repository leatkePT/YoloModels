...
# ---------------------------------------------------------
# Main API Class: YOLO7t_GlobalWheat
# ---------------------------------------------------------
class YOLO7t_GlobalWheat:

    def __init__(self):
        self.name = 'YOLOv7_Tiny_GlobalWheat'
        ...
        self.baseline_model_weights = "../trained_models/yolo7t/wheathead/yolov7_tiny_Global_Wheat_Pretrained.weights.h5"

        self.log_dir = "../logs/oop_experiments/wheat_v7"
    ...
    def build(self, print_summary=True):
        ...
        model = build_yolo7_model(variant="tiny", input_shape=(640, 640, 3), num_classes=self.num_classes)
                ...
                for scale_idx in range(3):
                    ...
                    for anchor_idx in range(3):
                        ...
                        if max(max(rw, 1 / rw), max(rh, 1 / rh)) < anchor_ratio_thresh:
                            ...
                            offset_x = 1 if (grid_x - gi_x) > 0.5 else -1
                            offset_y = 1 if (grid_y - gi_y) > 0.5 else -1
                            for nx, ny in [(gi_x + offset_x, gi_y), (gi_x, gi_y + offset_y)]:
    ...
    def fine_tune(self, path_to_save, new_model=None, print_summary=False, new_fine_tune_epochs=None):
        ...
        new_model = build_yolo7_model(variant="tiny", input_shape=(640, 640, 3), num_classes=self.num_classes)
