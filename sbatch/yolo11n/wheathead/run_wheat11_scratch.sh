#!/bin/bash
#SBATCH --job-name=Wh11_Scr
#SBATCH --partition=ampere
#SBATCH --gpus=1
#SBATCH --cpus-per-task=8
#SBATCH --ntasks=1
#SBATCH --time=06:00:00
#SBATCH --output=logs/yolo11n_logs/wheat11_scratch_out_%j.txt
#SBATCH --error=logs/yolo11n_logs/wheat11_scratch_error_%j.txt

echo "============================================================"
echo "🚀 ΕΚΚΙΝΗΣΗ JOB: YOLO11-Nano Global Wheat (From Scratch)"
echo "============================================================"
...
if [ $? -ne 0 ]; then
    echo "❌ ΣΦΑΛΜΑ: Το TensorFlow δεν βλέπει την GPU! Το job διακόπτεται."
    exit 1
fi
echo "✅ Η GPU είναι ενεργή!"

echo "⏳ Ξεκινάει η εκπαίδευση From Scratch..."
python3 -u server_training/wheat11_from_scratch.py
