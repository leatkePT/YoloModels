...
# Προσθήκη των επίσημων COCO Metrics (Γραμμένα σε C)
...
# =======================================================
# 1. DATASET & SOTA EXPERIMENT SETTINGS
# =======================================================
TARGET_SIZE = (640, 640)
BATCH_SIZE = 16
TOTAL_EPOCHS = 100
...
EVAL_FREQ = 1  # Πλέον τρέχει ΣΕ ΚΑΘΕ ΕΠΟΧΗ γιατί είναι αστραπιαίο!

DATASET_NAME = "PASCAL_VOC_0712_11n"
LOG_DIR = "/home/l/leandrosk/logs/yolo11n_logs/pascal"
...
PASCAL_YEARS = ["VOC2007", "VOC2012"]
BASE_DIR = "./datasets/pascal_voc/VOCdevkit"

all_image_data = []
for year in PASCAL_YEARS:
    img_dir = os.path.join(BASE_DIR, year, "JPEGImages")
    ann_dir = os.path.join(BASE_DIR, year, "Annotations")
...
def load_raw_image_and_boxes(data_tuple):
    img_path, _ = data_tuple
    if img_path not in xml_cache: return None, None, None
    boxes, classes = xml_cache[img_path]
...
def letterbox_image(data_tuple):
    img, boxes, classes = load_raw_image_and_boxes(data_tuple)
    if img is None: return None, None, None
    shape = img.shape[:2]
    ...
    for b in boxes:
        transformed_boxes.append(
            [(((b[0] + b[2]) / 2) * r + left) / TARGET_SIZE[1], (((b[1] + b[3]) / 2) * r + top) / TARGET_SIZE[0],
             ((b[2] - b[0]) * r) / TARGET_SIZE[1], ((b[3] - b[1]) * r) / TARGET_SIZE[0]])
...
def build_targets_11(boxes, classes):
    ...
    for i in range(len(boxes)):
        ...
        offset_x = 1 if (cx * grid_size - grid_x) > 0.5 else -1
        offset_y = 1 if (cy * grid_size - grid_y) > 0.5 else -1
        target_cells = [(grid_x, grid_y), (grid_x + offset_x, grid_y), (grid_x, grid_y + offset_y)]
...
def build_dataset_generator(use_mosaic):
    def dataset_generator():
        split_index = int(len(all_image_data) * 0.8)
        for i, data_tuple in enumerate(all_image_data):
            is_val = 0 if i < split_index else 1
            if is_val == 0:
                if use_mosaic and random.random() < 0.5:
                    image, boxes, classes = load_mosaic_dataset(data_tuple)
                else:
                    image, boxes, classes = letterbox_image(data_tuple)
            ...
            classes_padded = np.zeros((100,), dtype=np.float32) - 1.0
...
def create_tf_dataset(use_mosaic):
    dataset = tf.data.Dataset.from_generator(
        build_dataset_generator(use_mosaic),
        output_signature=(
    ...
    train_ds = dataset.filter(lambda img, box, cls, t1, t2, t3, is_val: is_val == 0).map(
        lambda img, box, cls, t1, t2, t3, is_val: (img, t1, t2, t3)).map(
        augment_colors, num_parallel_calls=tf.data.AUTOTUNE).batch(BATCH_SIZE, drop_remainder=True).prefetch(
        tf.data.AUTOTUNE)

    val_ds = dataset.filter(lambda img, box, cls, t1, t2, t3, is_val: is_val == 1).map(
        lambda img, box, cls, t1, t2, t3, is_val: (img, box, cls, t1, t2, t3)).batch(BATCH_SIZE * 2,
...
train_dataset_mosaic, val_dataset_standard = create_tf_dataset(use_mosaic=True)
train_dataset_nomosaic, _ = create_tf_dataset(use_mosaic=False)


# =======================================================
# 3. LOSS & DECODER FUNCTIONS
# =======================================================
...
def decode_yolo11_outputs_sota(model_outputs):
    ...
    nms_out = tf.image.combined_non_max_suppression(
        ...
        iou_threshold=0.60, score_threshold=0.01, clip_boxes=False
    )
    y1, x1, y2, x2 = tf.split(nms_out.nmsed_boxes, 4, axis=-1)
    nms_boxes_cyxhwh = tf.concat([(x1 + x2) / 2, (y1 + y2) / 2, x2 - x1, y2 - y1], axis=-1)
    mask = tf.sequence_mask(nms_out.valid_detections, maxlen=50)
...
class CosineDecayWithWarmup(tf.keras.optimizers.schedules.LearningRateSchedule):
    def __init__(self, max_lr, warmup_epochs, total_epochs, steps_per_epoch):
        ...
        self.min_lr = tf.cast(max_lr * 0.1, tf.float32)
        self.warmup_steps = tf.cast(warmup_epochs * steps_per_epoch, tf.float32)
        self.total_steps = tf.cast(total_epochs * steps_per_epoch, tf.float32)
...
def train_yolo():
    ...
    LATEST_CHECKPOINT_PATH = f"{LOG_DIR}/pascal_11n_pretrained_latest_checkpoint.weights.h5"
    PRETRAINED_WEIGHTS_PATH = f"/home/l/leandrosk/weights/yolo11n_coco_pretrained.weights.h5"
    ...
    num_train_images = int(len(all_image_data) * 0.8)
    steps_per_epoch = max(1, num_train_images // BATCH_SIZE)

    lr_schedule = CosineDecayWithWarmup(max_lr=MAX_LEARNING_RATE, warmup_epochs=WARMUP_EPOCHS,
                                        total_epochs=TOTAL_EPOCHS, steps_per_epoch=steps_per_epoch)

    optimizer = tf.keras.optimizers.SGD(learning_rate=lr_schedule, momentum=0.937, weight_decay=0.0005, use_ema=True,
                                        ema_momentum=0.999)
    ...
    for epoch in range(start_epoch, TOTAL_EPOCHS):
        ...
        if epoch >= TOTAL_EPOCHS - CLOSE_MOSAIC_EPOCHS:
            if epoch == TOTAL_EPOCHS - CLOSE_MOSAIC_EPOCHS:
                print(f"\n⚠️ ΑΠΕΝΕΡΓΟΠΟΙΗΣΗ MOSAIC ΓΙΑ ΤΙΣ ΤΕΛΕΥΤΑΙΕΣ {CLOSE_MOSAIC_EPOCHS} ΕΠΟΧΕΣ!")
            active_train_dataset = train_dataset_nomosaic
        else:
            active_train_dataset = train_dataset_mosaic

        pbar_train = tqdm(active_train_dataset, desc=f"Epoch {epoch + 1}/{TOTAL_EPOCHS} [Train]", leave=True)
        ...
        # === ΓΡΗΓΟΡΟ EVALUATION ME PYCOCOTOOLS ===
        if (epoch + 1) % EVAL_FREQ == 0:
            ...
            pbar_val = tqdm(val_dataset_standard, desc=f"Epoch {epoch + 1}/{TOTAL_EPOCHS} [Eval]", leave=False)
            ...
            for val_images, val_boxes, val_classes, t1, t2, t3 in pbar_val:
                ...
                # Fetch as numpy
                y_true_boxes_np = val_boxes.numpy()
                y_true_classes_np = val_classes.numpy()
                y_pred_boxes_np = y_pred_dict["boxes"].numpy()
                y_pred_classes_np = y_pred_dict["classes"].numpy()
                y_pred_scores_np = y_pred_dict["confidence"].numpy()

                batch_size_current = y_true_boxes_np.shape[0]

                for b in range(batch_size_current):
                    img_id_counter += 1
                    images_info.append({"id": img_id_counter, "width": TARGET_SIZE[0], "height": TARGET_SIZE[1]})

                    # 1. Καταγραφή Ground Truth
                    for i in range(100):
                        cls = y_true_classes_np[b, i]
                        if cls == -1: continue
                        cx = y_true_boxes_np[b, i, 0] * TARGET_SIZE[0]
                        cy = y_true_boxes_np[b, i, 1] * TARGET_SIZE[1]
                        w = y_true_boxes_np[b, i, 2] * TARGET_SIZE[0]
                        h = y_true_boxes_np[b, i, 3] * TARGET_SIZE[1]
                        xmin, ymin = cx - w / 2.0, cy - h / 2.0
                        gt_annotations.append({
                            "id": ann_id_counter, "image_id": img_id_counter, "category_id": int(cls),
                            "bbox": [float(xmin), float(ymin), float(w), float(h)],
                            "area": float(w * h), "iscrowd": 0
                        })
                        ann_id_counter += 1

                    # 2. Καταγραφή Predictions
                    for i in range(50):
                        cls = y_pred_classes_np[b, i]
                        if cls == -1: continue
                        score = y_pred_scores_np[b, i]
                        cx, cy, w, h = y_pred_boxes_np[b, i]
                        xmin, ymin = cx - w / 2.0, cy - h / 2.0
                        dt_annotations.append({
                            "image_id": img_id_counter, "category_id": int(cls),
                            "bbox": [float(xmin), float(ymin), float(w), float(h)],
                            "score": float(score)
                        })

            # Υπολογισμός mAP με pycocotools (εξαιρετικά γρήγορα!)
            f = io.StringIO()
            with redirect_stdout(f):
                categories = [{"id": i, "name": name} for name, i in class_to_id.items()]
                coco_gt = COCO()
                coco_gt.dataset = {"images": images_info, "annotations": gt_annotations, "categories": categories}
                coco_gt.createIndex()
                ...
                if len(dt_annotations) > 0:
                    ...
                    cocoEval.accumulate()
                    cocoEval.summarize()
                    map_50_95 = float(cocoEval.stats[0])
                    map_50 = float(cocoEval.stats[1])
                else:
