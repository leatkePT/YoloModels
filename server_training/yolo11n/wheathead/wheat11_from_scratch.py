...
# =======================================================
# 1. DATASET & FROM SCRATCH EXPERIMENT SETTINGS
# =======================================================
TARGET_SIZE = (640, 640)
BATCH_SIZE = 16
TOTAL_EPOCHS = 100
...
DATASET_NAME = "Global_Wheat_11n_Scratch"
IMAGE_DIR = "./datasets/global_wheat/train"
ANNOT_PATH = "./datasets/global_wheat/train.csv"
...
# =======================================================
# 2. PREPROCESSING & MOSAIC PIPELINE (Wheat specifics)
# =======================================================
grouped_boxes = {}
if os.path.exists(ANNOT_PATH):
    with open(ANNOT_PATH, mode='r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            xmin, ymin, w, h = bbox
            grouped_boxes[img_id].append([xmin, ymin, xmin + w, ymin + h])

all_image_ids = list(grouped_boxes.keys())
if len(all_image_ids) == 0:
    print(f"\n❌ ΣΦΑΛΜΑ: Δεν βρέθηκαν εικόνες/annotations στο {ANNOT_PATH}!")
    sys.exit(1)
print(f"\n✅ Βρέθηκαν {len(all_image_ids)} εικόνες για εκπαίδευση στο Global Wheat.")


def load_raw_image_and_boxes(img_id):
    img_path = os.path.join(IMAGE_DIR, img_id + ".jpg")
    if not os.path.exists(img_path) or img_id not in grouped_boxes:
...
def load_mosaic_dataset(img_id):
    ...
    indices = [img_id] + random.sample(all_image_ids, 3)
    ...
    for i, current_id in enumerate(indices):
        ...
        h, w, _ = img.shape
        if i == 0:
            x1_a, y1_a, x2_a, y2_a = 0, 0, xc, yc; x1_b, y1_b, x2_b, y2_b = w - xc, h - yc, w, h
        elif i == 1:
            x1_a, y1_a, x2_a, y2_a = xc, 0, w_target, yc; x1_b, y1_b, x2_b, y2_b = 0, h - yc, w_target - xc, h
        elif i == 2:
            x1_a, y1_a, x2_a, y2_a = 0, yc, xc, h_target; x1_b, y1_b, x2_b, y2_b = w - xc, 0, w, h_target - yc
        elif i == 3:
            x1_a, y1_a, x2_a, y2_a = xc, yc, w_target, h_target; x1_b, y1_b, x2_b, y2_b = 0, 0, w_target - xc, h_target - yc
    ...
    for i, img_id in enumerate(all_image_ids):
        ...
        if is_val == 0:
            if random.random() > 0.3:
                ...
                image, boxes, classes = letterbox_image(img_id)
        ...
        MAX_BOXES = 300
        boxes_padded, classes_padded = np.zeros((MAX_BOXES, 4), dtype=np.float32), np.zeros((MAX_BOXES,), dtype=np.float32) - 1.0
...
train_dataset = dataset.filter(lambda img, box, cls, t1, t2, t3, is_val: is_val == 0).map(
    lambda img, box, cls, t1, t2, t3, is_val: (img, t1, t2, t3)).map(augment_colors, num_parallel_calls=tf.data.AUTOTUNE).batch(
...
val_dataset = dataset.filter(lambda img, box, cls, t1, t2, t3, is_val: is_val == 1).map(
    lambda img, box, cls, t1, t2, t3, is_val: (img, box, cls, t1, t2, t3)).batch(BATCH_SIZE, drop_remainder=True).prefetch(
    tf.data.AUTOTUNE)
...
def compute_yolo11_loss(targets, model_outputs):
    ...
    for i, grid_pred in enumerate(model_outputs):
        ...
        cls_loss = tf.reduce_mean(bce(t_cls, cls_preds))
        ...
        ciou = bbox_ciou(tf.stack([pred_cx, pred_cy], axis=-1), tf.stack([pred_w, pred_h], axis=-1), t_boxes[..., 0:2],
                         t_boxes[..., 2:4])
        box_loss = tf.reduce_sum((1.0 - ciou) * mask) / (tf.reduce_sum(mask) + 1e-7)
...
def train_yolo():
    ...
    print("\n" + "=" * 60 + f"\n🎬 ΕΝΑΡΞΗ SOTA ΕΚΠΑΙΔΕΥΣΗΣ: YOLO11-Nano στο {DATASET_NAME}\n" + "=" * 60)
    ...
    lr_schedule = CosineDecayWithWarmup(max_lr=MAX_LEARNING_RATE, warmup_epochs=5, total_epochs=TOTAL_EPOCHS,
                                        steps_per_epoch=170)
    optimizer = tf.keras.optimizers.Adam(learning_rate=lr_schedule, use_ema=True, ema_momentum=0.999)
    ...
    for epoch in range(TOTAL_EPOCHS):
        ...
        for batch_images, t1, t2, t3 in pbar_train:
            ...
            pbar_train.set_postfix(Loss=f"{train_loss_total / num_train_batches:.4f}",
                                   LR=f"{lr_schedule(optimizer.iterations):.2e}")
        ...
        print(
            f"📊 Αποτελέσματα -> Train: {train_loss_total / max(1, num_train_batches):.4f} | Val: {val_loss_total / max(1, num_val_batches):.4f} | mAP@0.50: {map_50:.4f} | mAP@0.50:0.95: {map_50_95:.4f} | Χρόνος: {epoch_time:.1f}s")
        ...
        model.save_weights(f"{LOG_DIR}/wheat_11n_scratch_latest_checkpoint.weights.h5")

    model.save_weights(f"{LOG_DIR}/yolo11_nano_{DATASET_NAME}_final.weights.h5")
    print(f"\n🎉 Η εκπαίδευση ολοκληρώθηκε! Το αρχείο είναι έτοιμο.")
